 ...
 * ─────────────────────
 * All geographical hierarchy endpoints.
 * Every route requires a valid API key.
 *
 * GET /api/v1/geo/states
 * GET /api/v1/geo/states/:stateId/districts
 * GET /api/v1/geo/districts/:districtId/subdistricts
 * GET /api/v1/geo/subdistricts/:subdistrictId/villages
 * GET /api/v1/geo/village/:villageId
 */
...
// All geo routes require a valid API key
router.use(authenticateApiKey);

// ── GET /states ───────────────────────────────────────────────────
// Returns all states. Simple list, no pagination needed (only 36 max).
...
// ── GET /subdistricts/:subdistrictId/villages ─────────────────────
// Returns villages with pagination (can be large — up to thousands)
router.get("/subdistricts/:subdistrictId/villages", async (req, res) => {
  try {
    ...
    // Format each village with full address for dropdown use
...
// Get a single village with its full hierarchy
router.get("/village/:villageId", async (req, res) => {
  try {
    ...
    console.error("Get village error:", err);
